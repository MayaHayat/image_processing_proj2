...
def render_mesh_on_frame(frame, mesh, rvec, tvec, K, dist_coeffs=None, scale=1.0, offset=(0, 0, 0)):
    ...
    # 4. BRIGHT LIGHTING CONFIGURATION
    light_dir = np.array([0.0, 0.0, -1.0]) # Light comes from the camera
    light_dir /= np.linalg.norm(light_dir)
    
    # 5. HIGH AMBIENT CONSTANT
    # Setting this to 0.7 means the colors will never drop below 70% brightness
    ...
    for _, face_idx, face in triangles_with_depth:
        ...
        # --- DOUBLE-SIDED DIFFUSE CALCULATION ---
        # Use abs() so that even if the normal is flipped, the face is lit
        diffuse_intensity = abs(np.dot(normal, -light_dir)) 
        
        # --- HIGH-GLOSS SPECULAR ---
        view_dir = np.array([0, 0, -1]) 
        reflect_dir = light_dir - 2 * np.dot(light_dir, normal) * normal
        specular_intensity = pow(max(0, np.dot(reflect_dir, view_dir)), 32) 

        # Base color from your PLY script
        base_color = np.mean(vertex_colors[face], axis=0)
        
        # 6. BRIGHTNESS MIXING
        # Ambient (0.7) + Diffuse (weighted at 0.3) = max 1.0 intensity
        lighting_factor = ambient_intensity + (0.3 * diffuse_intensity)
        
        # Apply lighting and add bright white specular "glints"
        ...
        # Draw seams (subtle dark outline)
...
def main():
    ...
    out = cv2.VideoWriter('part3_output_texture.mp4', fourcc, fps, (frame_width, frame_height))
    
    print(f"\n=== Processing Video ===")
    print(f"Resolution: {frame_width}x{frame_height}, FPS: {fps}")
    
    # Mesh rendering parameters
    mesh_scale = (w_ref / mesh_size ) * 0.5 # Scale mesh to fit on the tracked plane
    mesh_offset = ((w_ref / 2) - 200, (h_ref / 2) - 500, -w_ref / 3)  # Position above the plane
